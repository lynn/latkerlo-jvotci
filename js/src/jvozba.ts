...
export function getRafsiForRafsi(
  ...
  isFirst: boolean,
  isLast: boolean,
  consonants: ConsonantSetting,  // TODO: defaults?
  glides: boolean
): [string, number][] {
...
export function getRafsiListList(
  ...
  const listOfRafsiLists: [string, number][][] = [];
  for (let [i, valsi] of valsiList.entries()) {
    let rafsiList: [string, number][] = [];
    const isFirst = i === 0;
...
export function combine(
  ...
  if (
    BANNED_TRIPLES.includes(lujvoFinal + rafsi.slice(0, 2))
  ) {
    return null;
  }
  ...
  rafsiListList[0].forEach((rafsi0) => {
    rafsiListList[1].forEach((rafsi1) => {
      ...
      const result = combine(
        rafsi0[0],
        rafsi1[0],
        rafsi0[1], // TODO: Bring back numConsonants. This is confusing.
        rafsi1[1],
        score(rafsi0[0]),
        [[0, stripHyphens(rafsi0[0]).length]],
        tosmabruType,
        generateCmevla,
